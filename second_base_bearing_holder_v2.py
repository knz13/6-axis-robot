...
def main():
    ...
    width = 37
    inner_length = 39
    total_length = inner_length + 10
    height = 4.8

    square = create_centered_rectangle(width, total_length, height)

    # dimensions
    base_cylinder_radius = 95

    hole_distance = 8.5 + m5_size*2
    nut_length = 10
    nut_width = 6
    nut_depth = 4
    triangle_height = 8
    outside_thickness = 8
    bases_distance = 53 + height
    extra_height_top = 8
    top_side_height = bearing_outer_radius*2 + extra_height_top

    triangle_sides = 50 + outside_thickness
    

    triangle = create_centered_triangle(triangle_sides, bases_distance + height + top_side_height, "Triangle")

    
    hole_bottom_distance = -(math.sqrt(3)/2*triangle_sides)/3 + outside_thickness + 12
    #triangle = make_hole(triangle, m5_size*2, triangle_height, (0,hole_bottom_distance,0))

    #triangle = make_hole(triangle, m5_size*2, triangle_height, (0,hole_bottom_distance+hole_distance,0))

    triangle.Placement = App.Placement(App.Vector(0,-hole_bottom_distance - base_cylinder_radius+10,0),App.Rotation(App.Vector(0,0,1),0))

    cylinder = create_hollow_cylinder(base_cylinder_radius + 1, 1,bases_distance + triangle_height + bearing_outer_radius/3, (0,0,0))
    
    triangle = cut(triangle, cylinder)

    square.Placement = App.Placement(App.Vector(0,-hole_bottom_distance - base_cylinder_radius+10,0),App.Rotation(App.Vector(0,0,1),0))

    triangle = join_parts(triangle,square)

    # make a cut on the top to check length

    top_cut = create_centered_rectangle(HOLE_INF, HOLE_INF, 0.001)

    top_cut.Placement = App.Placement(App.Vector(0,0,bases_distance),App.Rotation(App.Vector(0,0,1),0))

    triangle = cut(triangle, top_cut)


    # outside cut 

    cylinder = create_hollow_cylinder(HOLE_INF,base_cylinder_radius + outside_thickness,HOLE_INF, (0,0,-HOLE_INF/2))
    ...
    triangle = make_hole(triangle, bearing_inner_radius*2, HOLE_INF, (0,0,bases_distance  + bearing_outer_radius - tolerance*2),through_hole=True,hole_rotation=[(0,90,0),(90,0,0)])

    # bearing hole

    bearing_square = create_centered_rectangle(HOLE_INF, bearing_width,  bearing_outer_radius*2, "BearingSquare")

    bearing_square.Placement = App.Placement(App.Vector(0,2,bases_distance),App.Rotation(App.Vector(0,0,1),0))

    triangle = cut(triangle, bearing_square)

    # cut a square in front

    square = create_centered_rectangle(HOLE_INF, 20,  HOLE_INF, "Square")

    square.Placement = App.Placement(App.Vector(0,(2* (math.sqrt(3)/2*triangle_sides)/3) - 10,triangle_height + 1),App.Rotation(App.Vector(0,0,1),0))

    triangle = cut(triangle, square)
