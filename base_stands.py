...
base_stand_hole_distance = 6.61 + 8.5
base_stand_width = 30 + m5_size*2
base_stand_length = base_stand_hole_distance + 20
base_stand_height = 100
base_stand_space_for_screws = 30
base_stand_width_for_screws = 16
base_stand_screw_depth = 7
def main():

    doc = App.ActiveDocument

    # create the base stands as a rectangle

    

    base_stands = create_centered_rectangle(base_stand_length, base_stand_width , base_stand_height)

    # create two holes in the base stands

    for i in range(2):
        base_stands = make_hole(base_stands, m5_size*2, base_stand_screw_depth*2, ((lerp([0,1],[-1,1])(i)) * base_stand_hole_distance/2,0,base_stand_height),through_hole=True)
    
    # make a cut below the holes    
        
    base_stands_cut = create_centered_rectangle(base_stand_length, base_stand_width_for_screws , base_stand_space_for_screws)

    base_stands_cut.Placement = App.Placement(App.Vector(0, 0, base_stand_height - base_stand_space_for_screws - base_stand_screw_depth), App.Rotation(App.Vector(0,0,1),0))

    base_stands = cut(base_stands, base_stands_cut)
